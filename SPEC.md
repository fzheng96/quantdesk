...
- class YahooSource: fetch_ohlcv(ticker: str, start: date, end: date) -> DataFrame with columns open, high, low, close, volume and a DatetimeIndex. Uses the unofficial Yahoo v8 chart endpoint https://query1.finance.yahoo.com/v8/finance/chart/<SYMBOL> (period1/period2 unix seconds, interval=1d) via httpx with a 20s timeout and 2 retries (5xx and 429 retry; other 4xx fail fast). The close column carries Yahoo's adjusted close when present; open/high/low/volume stay unadjusted, acceptable because the pipeline consumes closes only. Bar timestamps convert through US Eastern to trading dates, so non-US listings are out of scope. Raises DataError on error payloads, non-JSON bodies, or missing arrays.
- class ChainedSource: takes a sequence of sources and tries each in order; only DataError falls through to the next source, and when all fail it raises one DataError naming each source's failure.
...
- get_prices(tickers: list[str], start, end, source=None, cache=None) -> wide DataFrame of close prices (columns = tickers). The default source is ChainedSource((YahooSource(), StooqSource())). Cache-first; fetches only missing ranges; forward-fills gaps up to 3 days; drops leading NaNs per column. A failed fetch degrades to cached data when the cache yields anything for the ticker (the weekend/no-new-bars case); a ticker with no usable data is dropped from the result, and DataError is raised only when every requested ticker fails.
